...
config/seed-hooks/<project>  optional executable that prepares one task worktree of that project before the agent is launched into it; LOCAL, gitignored, and NOT inherited by secondmate homes; a project with no file here spawns unchanged; see docs/configuration.md "Project worktree seeding hooks"
...
- `firstmate-fork-rebase` - load before updating firstmate in a home whose default branch carries its own commits on top of upstream, and whenever `/updatefirstmate` or a fleet-sync check reports this home skipped as diverged or behind with local commits it must keep.
